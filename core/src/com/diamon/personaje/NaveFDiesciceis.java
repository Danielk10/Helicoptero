...
public class NaveFDiesciceis extends Personaje {
	...
	public NaveFDiesciceis(Texture textura, Pantalla pantalla) {
	...
	public NaveFDiesciceis(TextureRegion texturaRegion, Pantalla pantalla) {
	...
	public NaveFDiesciceis(Array<AtlasRegion> texturaRegion, float tiempoAnimacion, PlayMode modo, Pantalla pantalla) {
		...
		if (x <= camara.position.x + Juego.ANCHO_PANTALLA / 2) {
			...
			if (tiempoCuadro / duracionDisparo >= 1) {
...

				{
					...
					disparar(BalaEnemigo.IZQUIERDO_ABAJO);

				}

				if (jugador.getY() <= y && jugador.getX() >= x + this.getWidth())

				{

					disparar(BalaEnemigo.DERECHO_ABAJO);
	...
	public void disparar(int lado) {
		...
		if (BalaEnemigo.IZQUIERDO_ABAJO == lado) {

			bala.setPosition(x, y);

		}

		if (BalaEnemigo.DERECHO_ABAJO == lado) {

			bala.setPosition(x + this.getWidth(), y);
